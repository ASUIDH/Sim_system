import os
import configparser
from Simlarity import GetModel,TestModel,GetGraph


cf = configparser.SafeConfigParser()
# read the parameters file.
#     cf.read("/usr/lzmExperiment/proxEmbed/paramsSet/pythonParamsConfig")
cf.read("D:\\pyprojects\\MD-system\\be\\Simlarity\\pythonParamsConfig", encoding="utf-8")
...
trainingDataFile = os.path.join(main_dir + '/', dataset_name + '.splits', 'train.' + suffix,
                                'train_' + class_name + '_' + index)  # the full path of training data file. This path will be generated by main_dir, dataset_name, suffix, class_name and index.
...
computeFunc = GetModel.get_batch_proxEmbedModel(
                         saveto,
                         word_dimension,
                         dimension,
                         h_output_method,
...
def test(query,candidates):
    tops = TestModel.compute_proxEmbedbatch2(
        wordsEmbeddings,
    ...
    return tops
def testWithMd5(md5):
    name_id, id_name, id_type = GetGraph.getId(os.path.join(main_dir,dataset_name,"graph.node"))
    query = name_id[md5]
    candidates = []
    for id in id_name:
